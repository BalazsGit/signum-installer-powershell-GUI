...
POWERSHELL_ZIP="PowerShell-$POWERSHELL_VERSION-linux-x64.tar.gz"
POWERSHELL_UNZIP="PowerShell-$POWERSHELL_VERSION-linux-x64"
...
if [[ -f "$POWERSHELL_DIR/$POWERSHELL_ZIP" ]]; then
    ...
    wget -O "$POWERSHELL_DIR/$POWERSHELL_ZIP" "$POWERSHELL_URL"
fi
...
"$POWERSHELL_EXEC" "./signum-installer.ps1"
